  ...
  (req, res) => {
    try {
      ...
      if (!name || !creatorName || !creatorId) {
        return res
          .status(400)
          .json({ message: "Name, creator name, and creator ID are required" });
      }

      const sql = `
              INSERT INTO podcast (
                  name, listens, author_id, creator_id, show_title, description, creator_name,
                  episodes, image, reviews, more_like_this, genre, season, created_at, admin_id
              ) 
              VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
          `;
      ...
      const values = [
        ...
        season && !isNaN(season) ? parseInt(season, 10) : 1, // Default to 1 if not provided
...
exports.updatePodcastById = (req, res) => {
  ...
  db.query(
    "UPDATE podcast SET name = ?, listens = ?, author_id = ? WHERE id = ?",
    [name, listens, authorId, id],
    (err, result) => {
      if (err) {
...
exports.getAllPodcasts = (req, res) => {
  db.query(
    "SELECT podcast.*, author.name as author_name, author.email as author_email FROM podcast JOIN author ON podcast.author_id = author.id",
    (err, result) => {
      if (err) {
        return res.status(500).json({ message: "Database error", error: err });
      }
...
exports.getPodcastsByAuthor = (req, res) => {
  ...
  db.query(
    "SELECT * FROM podcast WHERE author_id = ?",
    [authorId],
    (err, result) => {
      if (err) {
        return res.status(500).json({ message: "Database error", error: err });
...
exports.createEpisode = (req, res) => {
  upload.fields([
    ...
    if (uploadErr) {
      return res
        .status(400)
        .json({ message: "File upload failed", error: uploadErr });
    }

    const { title, podcastId, name, season, episode, description, creator } =
      req.body;

    const serverUrl = `${req.protocol}://${req.get("host")}`;
    const imagePath =
      req.files && req.files.image
        ? `${serverUrl}/${req.files.image[0].path.replace(/\\/g, "/")}`
        : null;
    const audioPath =
    ...
    if (!title || !podcastId || !audioPath) {
      return res
        .status(400)
        .json({ message: "Title, podcast ID, and audio file are required" });
    }

    const audioFilePath = path.resolve(audioPath);

    getAudioDurationInSeconds(audioFilePath)
      .then((duration) => {
        const query = `
        ...
        db.query(query, values, (err, result) => {
          ...
          res.status(201).json({
            ...
            data: {
              ...
              creator,
              image: imagePath,
              audio: `${serverUrl}/${audioPath.replace(/\\/g, "/")}`,
            },
          });
        });
      })
      .catch((err) => {
        console.error("Error reading audio file duration:", err);
        return res
          .status(500)
          .json({ message: "Error reading audio metadata", error: err });
      });
  });
};
...
exports.getEpisodesByPodcast = (req, res) => {
  ...
  db.query(
    "SELECT * FROM episode WHERE podcast_id = ?",
    [podcastId],
    (err, result) => {
      if (err) {
        return res.status(500).json({ message: "Database error", error: err });
      ...
      if (result.length === 0) {
        return res
          .status(404)
          .json({ message: "No episodes found for this podcast" });
      }
      res.status(200).json(result);
    }
  );
...
exports.updateAuthor = (req, res) => {
  ...
  db.query(
    "UPDATE author SET name = ?, email = ? WHERE id = ?",
    [name, email, id],
    (err, result) => {
      if (err) {
        return res.status(500).json({ message: "Database error", error: err });
...
exports.updateEpisode = (req, res) => {
  ...
  db.query(
    "UPDATE episode SET title = ?, duration = ? WHERE id = ?",
    [title, duration, id],
    (err, result) => {
      if (err) {
        return res.status(500).json({ message: "Database error", error: err });
...
exports.getAllPodcastsOfCreator = (req, res) => {
  ...
  db.query(sql, [creatorId], (err, result) => {
    ...
    if (result.length === 0) {
      return res
        .status(404)
        .json({ message: "No podcasts found for this creator" });
    }
...
exports.getAllEpisodesOfAllPodcasts = (req, res) => {
  ...
  db.query(sql, (err, result) => {
    ...
    if (result.length === 0) {
      return res
        .status(404)
        .json({ message: "No episodes found for any podcast", result: [] });
    }
...
exports.getPodcastByGenre = (req, res) => {
  ...
  db.query(sqlGetPodcastIds, [genre_id], (err, genrePodcasts) => {
    ...
    if (genrePodcasts.length === 0) {
      return res
        .status(404)
        .json({ message: "No podcasts associated with this genre" });
    }
    ...
    db.query(sqlGetPodcasts, [podcastIds], (err, podcasts) => {
      ...
      if (podcasts.length === 0) {
        return res
          .status(404)
          .json({ message: "No podcasts found for this genre" });
      }
  ...
  db.query(fetchQuery, [id], (fetchErr, fetchResult) => {
    ...
    db.query(deleteQuery, [id], (deleteErr, deleteResult) => {
      if (deleteErr) {
        return res
          .status(500)
          .json({ message: "Database error", error: deleteErr });
      }
...
exports.deletePodcastById = (req, res) => {
  const { id } = req.params; // `id` is taken from the request parameters

  const sql = "DELETE FROM podcast WHERE podcast_id = ?"; // Use `podcast_id` to match the podcast
